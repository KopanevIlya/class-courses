...
const canLogin = true // для примера
...
const faqs = ref([
  { q: 'Нужны ли специальные знания?', a: 'Нет! Всё объясняется с нуля. Главное — желание учиться и развиваться.', open: false },
  { q: 'Как проходит обучение?', a: 'Все занятия проходят онлайн, после каждого урока запись выкладывается на платформу. Практические задания, поддержка и чаты доступны для всех участников.', open: false },
  { q: 'Что если возникнут вопросы?', a: 'Всегда можно обратиться ко мне напрямую или в чат поддержки.', open: false },
  { q: 'Какие проекты будем делать?', a: 'Портфолио, интернет-магазин, чат, REST API, и многое другое.', open: false },
])
  ...
  <div class="min-h-screen flex flex-col bg-gradient-to-br from-blue-50 via-white to-blue-100 overflow-hidden relative font-sans">
    ...
    <header class="relative z-10 py-6 px-4 flex justify-between items-center max-w-7xl mx-auto w-full">
      <div class="flex items-center gap-3 animate-slide-down">
        <img src="https://placehold.co/70x70/3b82f6/fff?text=LOGO" class="h-[70px] rounded-xl shadow" alt="CLASS" />
    ...
    <section class="relative z-10 flex flex-col md:flex-row items-center justify-center px-4 py-16 gap-10 max-w-7xl mx-auto w-full">
      <div class="max-w-xl">
        <h1 class="text-4xl md:text-5xl font-extrabold text-blue-900 mb-6 leading-tight">
          CLASS —
          <span class="typewriter text-blue-600">курс фуллстек-разработки</span>
          нового поколения
        </h1>
        ...
        <div class="mt-8 text-blue-900 font-bold text-xl animate-zoom-in">
    ...
    <!-- PRICE BLOCK -->
    <section class="relative z-10 max-w-5xl mx-auto px-4 py-8 mt-10">
      <div class="relative rounded-2xl p-1 bg-gradient-to-r from-blue-400 via-blue-200 to-blue-400 animate-glow">
        <div class="bg-white rounded-2xl p-8 md:p-12 flex flex-col md:flex-row items-center gap-8">
          <div class="flex-1">
            ...
            <div class="text-2xl font-bold mb-2 flex items-end gap-3">
              ...
              <span class="text-blue-400 line-through text-xl">45 000 руб.</span>
            </div>
          ...
          <div class="flex-1 flex flex-col items-center">
            ...
            <div class="flex gap-2 text-2xl font-bold text-blue-900 bg-blue-100 rounded-xl px-6 py-3 animate-bounce-in">
      ...
      <div class="space-y-4">
        ...
        >
          ...
          <transition name="fade">
            <p v-if="item.open" class="text-blue-800 mt-2">{{ item.a }}</p>
          </transition>
        </div>
      </div>
    </section>
...
.typewriter {
  ...
  overflow: hidden;
  border-right: .13em solid #3b82f6;
  white-space: nowrap;
